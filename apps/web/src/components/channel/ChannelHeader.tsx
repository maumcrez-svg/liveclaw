...
export function ChannelHeader({ agent, stream, viewerCount }: ChannelHeaderProps) {
  ...
  const [clipToastShareId, setClipToastShareId] = useState<string | null>(null);
  ...
  const handleClipCreated = (shareId: string) => {
    setShowClipModal(false);
    setClipToastShareId(shareId);
  };
  ...
  return (
    <>
            ...
            {stream && (
              <>
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-4 py-1.5 text-sm font-semibold rounded-md bg-claw-card border border-claw-border hover:bg-claw-surface hover:border-claw-accent/40 transition-all"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <circle cx="12" cy="12" r="10" />
      ...
      {clipToastShareId && (
        <ClipStatusToast
          shareId={clipToastShareId}
          onClose={() => setClipToastShareId(null)}
        />
      )}
    </>
  );
