...
const CATEGORY_KEYWORDS: Record<NewsCategory, string[]> = {
  base_ecosystem: ['base', 'base chain', 'base l2', 'base network', 'coinbase l2'],
  defi: ['defi', 'decentralized finance', 'liquidity', 'yield', 'tvl', 'lending', 'dex', 'amm', 'aerodrome', 'uniswap'],
  nft: ['nft', 'mint', 'collection', 'opensea', 'zora'],
  builder: ['deploy', 'launch', 'build', 'developer', 'hackathon', 'grant'],
  launch: ['mainnet', 'release', 'live on base', 'launches on', 'deployed'],
  onchain: ['onchain', 'on-chain', 'smart contract', 'transaction', 'gas'],
  governance: ['governance', 'dao', 'vote', 'proposal', 'optimism', 'op stack'],
  culture: ['community', 'farcaster', 'social', 'meme', 'degen', 'toshi', 'brett'],
  market: ['price', 'market cap', 'trading', 'volume', 'exchange'],
  infrastructure: ['bridge', 'rollup', 'sequencer', 'l2', 'layer 2', 'op stack', 'superchain'],
};

function classifyCategory(title: string, summary: string): NewsCategory {
  const text = `${title} ${summary}`.toLowerCase();
  let bestCategory: NewsCategory = 'base_ecosystem';
  let bestScore = 0;
...
export async function fetchRssFeed(source: RssSource): Promise<RawArticle[]> {
  try {
    const res = await fetch(source.url, {
      headers: { 'User-Agent': 'BasePulse/1.0' },
    ...
    for (const a of articles) {
      a.layer = source.layer === 'official' ? 'official' : 'official';
    }
