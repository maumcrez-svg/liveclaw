...
async function extractOpinionsLLM(
  ...
  const storySegments = segments
    .filter(s => ['builder_spotlight', 'signal_analysis', 'pulse_check', 'chain_radar', 'social_pulse'].includes(s.type))
...
function extractOpinionsRegex(
  ...
  for (const seg of segments) {
    if (!['builder_spotlight', 'signal_analysis', 'pulse_check'].includes(seg.type)) continue;
