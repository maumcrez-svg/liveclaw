...
echo "=== SPACEX MISSION CONTROL CONTAINER ==="
...
for attempt in 1 2 3 4 5; do
  if pactl info > /dev/null 2>&1; then
    pactl load-module module-null-sink sink_name=spacex_voice sink_properties=device.description="SpaceXVoice" 2>/dev/null
    pactl set-default-sink spacex_voice 2>/dev/null
    export PULSE_SINK=spacex_voice
    if pactl list short sinks 2>/dev/null | grep -q spacex_voice; then
      echo "[OK] PulseAudio sink: spacex_voice (attempt $attempt)"
      AUDIO_INPUT="-f pulse -i spacex_voice.monitor"
      break
    fi
  fi
...
# Ensure Chromium can access PulseAudio
export PULSE_SERVER=unix:/run/pulse/native
...
echo "[FFmpeg] Streaming to $STREAM_URL"
