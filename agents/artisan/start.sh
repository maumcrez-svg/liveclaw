...
# Clean any duplicate artisan_voice sinks before creating a fresh one
for mod in $(pactl list short modules | grep artisan_voice | awk '{print $1}'); do
  pactl unload-module "$mod" 2>/dev/null || true
done

pactl load-module module-null-sink sink_name=artisan_voice \
  sink_properties=device.description="ArtisanVoice" rate=44100 2>/dev/null || true
...
export PULSE_SINK=artisan_voice

echo "PulseAudio ready (PULSE_SINK=artisan_voice)"
...
# Start the ArtisanAI agent
echo "[2/4] Starting ArtisanAI agent..."
cd "$(dirname "$0")"
node dist/index.js &
AGENT_PID=$!
sleep 5
...
echo "=== ArtisanAI is LIVE ==="

# Cleanup handler
cleanup() {
    echo "Shutting down ArtisanAI..."
